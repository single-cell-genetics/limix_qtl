    ...
    for feature_id in feature_list:
        ...
        ########################################################################################################################################################
            ...
            if(not np.isnan(zeroValue)):
                ...
                phenotype_c = utils.force_normal_distribution(phenotype_ds.values,method=gaussianize_method) if gaussianize_method is not None else phenotype_ds.values
                phenotype_c = phenotype_c.astype(float)
            ...
            if randomeff_mix:
                ...
                ##Optimization of mixing parameters only happens on the continous part. (might not be ideal).
                ## high rho means a strong effect of the kinship matrix, low rho is a strong effect of the other matrix.
                if(not np.isnan(zeroValue)):
                    mixingParameters = utils.rhoTestBF(best=None, phenotype = phenotype_c, cov_matrix=cov_matrix_c, Sigma_qs=Sigma_qs_c, mixed=mixed, lastMove=None, rhoArray = rho1, verbose = False)
                else :
                    #mixingParameters = utils.rhoTest(best=None, phenotype = phenotype_c, cov_matrix=cov_matrix, Sigma_qs=Sigma_qs, mixed=mixed, lastMove=None, rhoArray = rho1, verbose = True)
                    mixingParameters = utils.rhoTestBF(best=None, phenotype = phenotype_c, cov_matrix=cov_matrix, Sigma_qs=Sigma_qs, mixed=mixed, lastMove=None, rhoArray = rho1, verbose = False)
                ...
                if(not np.isnan(zeroValue)):
                    glmm = LMM(phenotype_b, cov_matrix, Sigma_qs[feature_best_rho])
                ...
                if(not np.isnan(zeroValue)):
                    glmm = LMM(phenotype_b, cov_matrix, QS)
                    lmm = LMM(phenotype_c, cov_matrix_c, QS_c)
                else :
                    lmm = LMM(phenotype_c, cov_matrix, QS)
                if not mixed:
                    lmm.delta = 1
                    lmm.fix('delta')
                    glmm.delta = 1
            ...
            if regressCovariatesUpfront:
                #pdb.set_trace();
                ##Correction is only performed on the continous part. (might not be ideal).
                if(not np.isnan(zeroValue)):
...
#                test if the covariates, kinship, snp and phenotype are in the same order
                ...
                if(not np.isnan(zeroValue)):
                    ...
                    Gc = Gc.astype(float)
                    Gc_index = snp_df.columns
                    
                    scannerOut = flmm.fast_scan(Gc, verbose=False)
                    ...
                    scannerOutb = fglmm.fast_scan(G, verbose=False)
                    alt_glmls = scannerOutb['lml']
                    effsizes_g = scannerOutb['effsizes1']
                    var_effsizes_g_se = scannerOutb['effsizes1_se']
                    var_pvalues_g = utils.lrt_pvalues(fglmm.null_lml(), alt_glmls)
                    ## var_pvalues_g = utils.lrt_pvalues(null_glml, alt_glmls) (original 
                    ##Here we need to do some magic, to get a proper effect size and pvalue. First check what this looks like in the full model.
